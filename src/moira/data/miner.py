...
Minero robusto para construir dataset historico con moira.features.build_climate_data.
...
Si quieres probar pocas fechas:
  PYTHONPATH=src python -m moira.data.miner --start 1983-01-01 --end 1983-01-31 --output ./data/processed/dataset_test.csv
"""
...
def ensure_project_root_on_path(script_file: str) -> None:
    ...
    candidate_paths = [src_root, repo_root, Path.cwd().resolve()]
...
def import_weather_module(module_name: str, script_file: str):
    ...
    try:
        ...
        if not module_name.startswith("moira."):
            try:
                return importlib.import_module(f"moira.features.{module_name}")
            except Exception:
                pass
        raise RuntimeError(
...
def main() -> int:
    ...
    ap.add_argument("--module", default="moira.features.build_climate_data")
    ...
    ap.add_argument("--output", default="./data/processed/sprint2.csv")
    ap.add_argument("--failed-output", default="./data/processed/sprint2_failed_rows.csv")
    ap.add_argument("--metadata-output", default="./data/processed/sprint2_metadata.json")
    ap.add_argument("--backup-dir", default="./data/processed/backups")
