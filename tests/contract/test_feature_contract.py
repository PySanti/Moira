...
Uso recomendado:
  PYTHONPATH=src python -m tests.contract.test_feature_contract \
    ...
    --strict false \
    --mode train_mode \
    --out-dir ./reports/contract-tests/feature_contract_test

Uso rapido:
  PYTHONPATH=src python -m tests.contract.test_feature_contract --n-samples 50
...
- No puede probar por sí solo el origen real de cada dato aguas arriba.
   ...
   PYTHONPATH=src python -m tests.contract.test_feature_contract --module moira.features.build_climate_data --function get_weather_features --city "new york" --start 1980-01-01 --end 2025-12-12 --n-samples 50 --strict True
"""
...
def load_function(module_name: str, function_name: str):
    ...
    try:
        ...
        if not module_name.startswith("moira."):
            try:
                module = importlib.import_module(f"moira.features.{module_name}")
            except Exception:
                raise RuntimeError(
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", default="./reports/contract-tests/feature_contract_test")
